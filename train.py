...
from model.yolov3_l import Yolov3_L
...
class Trainer(object):
    ...
    def __load_model_weights(self, weight_path, resume):
        if resume:
            last_weight = os.path.join(
                weight_path, f"last{self.model_postfix}.pt")
    ...
    def __save_model_weights(self, epoch, mAP):
        ...
        best_weight = os.path.join(
            self.weight_path, f"best{self.model_postfix}.pt")
        last_weight = os.path.join(
            self.weight_path, f"last{self.model_postfix}.pt")
        chkpt = {'epoch': epoch,
        ...
        if epoch > 0 and epoch % 10 == 0:
            torch.save(chkpt, os.path.join(self.weight_path,
                       f'backup_epoch{epoch}{self.model_postfix}.pt'))
        del chkpt
...
if __name__ == "__main__":
    ...
    parser.add_argument('--weight_path', type=str,
                        default='weights', help='weight directory path')
    parser.add_argument('--resume', action='store_true',
                        default=False,  help='resume training flag')
